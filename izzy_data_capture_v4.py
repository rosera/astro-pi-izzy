...
def get_magfield_data(Earth_magfld):

    Earth_magfld = sense.get_compass_raw()

    return (Earth_magfld)

def output_data(Temp, Humid, Presh, IZ_Long, IZ_Lat, TERRA_Magfld):

        print (time.strftime('%x %X'),Temp, Humid, Presh, str(IZ_Long), str(IZ_Lat), str(TERRA_Magfld))
...
try:
      while True:
            ...
            TERRA_Magfld = get_magfield_data(TERRA_Magfld)
            output_data(IZ_Temp, IZ_Humid, IZ_Presh, IZ_Long, IZ_Lat, TERRA_Magfld)
